...
def data_visualization_view(request):
    ...
    try :
        ...
        if user.groups.filter(name='Technicien').exists():
            # If the user is in the "Technicien" group, only show their files
...
def Information_view(request):
    ...
    # List of PDF file names that technicians can access
    technician_accessible_pdfs = ["guide pour la fonction de visualisation des données.pdf", "guide pour la fonction de téléchargement de fichier.pdf"]

    # Iterate through the files in the 'pdfs' directory
    for filename in os.listdir(pdfs_dir):
        if filename.endswith('.pdf'):
            pdf_path = os.path.join('/media/pdfs', filename)

            if is_admin or filename in technician_accessible_pdfs:
                pdfs.append({'name': filename, 'url': pdf_path})

   
    

    return render(request, 'information.html', { 'pdfs': pdfs })